...
import { AppButton, AppShell, Card, EmptyState, SectionTitle, theme } from './app-ui';
    ...
    <AppShell title="Call Room" subtitle={`Connect with ${partnerName}`}>
      ...
      {partner ? <>
        <Card style={{ backgroundColor: '#1f3b63' }}>
          <Text style={{ fontSize: 24, fontWeight: '800', color: '#fff' }}>Voice calling</Text>
          <Text style={{ color: '#dbe7f5', lineHeight: 20 }}>Voice calls still run app-to-app through the built-in WebRTC connection.</Text>
          <Text style={{ color: '#cbd5e1', lineHeight: 20, fontSize: 13 }}>Video now uses Google Meet from the dedicated video tab. For voice reliability, configure TURN credentials in `mobile-app/.env`.</Text>
          ...
          <View style={{ gap: 12 }}>
            ...
            <AppButton title="Open Google Meet Video" onPress={() => router.push('/video' as never)} leftIcon={<Video size={16} color="#fff" />} />
          </View>
        </Card>
